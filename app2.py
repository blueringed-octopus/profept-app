...
df_questions = df_filtered.iloc[:, 1:]

df_questions_rnm = df_filtered.iloc[:, 1:]
...
# Filtrando as colunas, transformando em linhas e renomeando a nova coluna para "Respostas"
...
# Mapeando os valores de 'Questões' para números de 1 a 30
...
fig = px.bar(freq_questions_melted, x='Questões', y='Frequência', color='Respostas', title="Frequência das Respostas por Questão", width=1000, height=600)
fig.update_xaxes(tickvals=list(range(0, 31)))
st.plotly_chart(fig)
...
new_column_names = {old_name: f"Q{new_name}" for old_name, new_name in zip(df_sidebar1.columns[1:], range(1, len(df_sidebar1.columns)))}
