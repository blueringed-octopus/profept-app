...
if subject == "Gênero e Sexualidade":
    ...
    df_gen_sex = df_filtered[df_filtered["Curso"] == course]
    df_gen_sex = df_gen_sex.iloc[:, 2:12]
    df_gen_sex = df_gen_sex.apply(pd.Series.value_counts).fillna(0).T.rename(columns={0: "Questões"})
    st.write(df_gen_sex)
    ...
    df_racism = df_filtered[df_filtered["Curso"] == course]
    df_racism = df_racism.iloc[:, 12:22]
    df_racism = df_racism.apply(pd.Series.value_counts).fillna(0).T.rename(columns={0: "Questões"})
    st.write(df_racism)
    ...
    df_laws = df_filtered[df_filtered["Curso"] == course]
    df_laws = df_laws.iloc[:, 22:32]
    df_laws = df_laws.apply(pd.Series.value_counts).fillna(0).T.rename(columns={0: "Questões"})
    st.write(df_laws)
