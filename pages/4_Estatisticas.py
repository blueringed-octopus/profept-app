...
df_stats_tests = pd.DataFrame({
    "Estatísticas": ["Estatística Descritiva", "Testes Estatísticos - Curso e Ano", "Testes Estatísticos - Indices de Conhecimento" ],
})
...
if stats_choice == "Estatística Descritiva":
#if stats_sidebar == "Estatística Descritiva":
    st.write("Esta seção apresenta as estatísticas descritivas dos dados coletados, incluindo médias, medianas, desvios padrão e outras medidas de tendência central e dispersão.")
    ...
    #    N='count'
    #).reset_index()
    # Calcula o grau de liberdade para cada grupo (N - 1)
    stats_df['Grau_de_Liberdade'] = stats_df['N'] - 1
...
elif stats_choice == "Testes Estatísticos - Curso e Ano":
    st.write("Esta seção apresenta os testes estatísticos realizados para verificar diferenças significativas entre os cursos e anos dos estudantes.")
